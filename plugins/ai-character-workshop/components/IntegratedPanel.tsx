...
export const IntegratedPanel: React.FC<IntegratedPanelProps> = ({
  ...
  useEffect(() => {
    if (isOpen && !isPositionInitialized) {
      ...
      if (autoSaved && autoSaved.data.name) {
        ...
        if (timeDiff < 24 * 60 * 60 * 1000) { // 24小时内的自动保存
          // 如果当前角色是空的，提示恢复
  ...
  }, [isOpen, isPositionInitialized, state.wizardCharacter.name, state.wizardCharacter.style?.genre, isZh]);
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginRight: 12 }}>
            <button
              ...
              title={isZh ? '保存当前角色' : 'Save current character'}
            >
            ...
            <button
              onClick={() => { setSavedCharacters(getSavedCharacters()); setShowSavePanel(!showSavePanel); }}
              style={{
                padding: '6px 12px',
                background: 'var(--ef-bg-tertiary)',
                border: '1px solid var(--ef-border)',
              ...
              title={isZh ? '加载已保存的角色' : 'Load saved character'}
            >
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: 20, position: 'relative' }}>
          ...
          {saveMessage && (
            <div style={{
              ...
              padding: '8px 16px',
              background: saveMessage.includes('恢复') || saveMessage.includes('Restore') ? 'var(--ef-warning-bg, rgba(245,158,11,0.15))' : 'var(--ef-success-bg, rgba(34,197,94,0.15))',
              border: `1px solid ${saveMessage.includes('恢复') || saveMessage.includes('Restore') ? 'var(--ef-warning, #f59e0b)' : 'var(--ef-success, #22c55e)'}`,
              borderRadius: 8,
              color: saveMessage.includes('恢复') || saveMessage.includes('Restore') ? 'var(--ef-warning, #f59e0b)' : 'var(--ef-success, #22c55e)',
              fontSize: 13,
              zIndex: 100,
              display: 'flex',
              alignItems: 'center',
              ...
              {saveMessage.includes('恢复') || saveMessage.includes('Restore') ? (
                <button
                  ...
                  style={{
                    padding: '4px 10px',
                    background: 'var(--ef-warning, #f59e0b)',
                    border: 'none',
                    borderRadius: 4,
                    color: 'white',
                    fontSize: 12,
            ...
            <div style={{
              ...
              <div style={{
                ...
                <button
                  ...
                  style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--ef-text-muted)', fontSize: 18 }}
                >
                  ✕
                </button>
              </div>
              <div style={{ flex: 1, overflow: 'auto', padding: 12 }}>
                {savedCharacters.length === 0 ? (
                  <div style={{ textAlign: 'center', padding: 40, color: 'var(--ef-text-muted)', fontSize: 13 }}>
                      ...
                      >
                        ...
                        {saved.data.style?.genre && (
                          <div style={{
                            ...
                            {(saved.data.tags || []).slice(0, 2).map(tag => (
                              <span key={tag} style={{
                                padding: '2px 6px',
                                background: 'var(--ef-bg-primary)',
                                borderRadius: 4,
